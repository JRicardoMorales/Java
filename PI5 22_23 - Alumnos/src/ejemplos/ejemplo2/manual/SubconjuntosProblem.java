...
public record SubconjuntosProblem(Integer index, Set<Integer> remaining) {
	...
	public List<Integer> actions() {
		...
		if(index < DatosSubconjuntos.getNumSubconjuntos()) {
			if(remaining.isEmpty()) // ya he cubierto todos los elementos del universo
				...
				if(rest.isEmpty()) {  
					// me interesa coger ese subconjunto porque el remaining me lo deja listo, vacío
					alternativas = List2.of(1);
				} else if(rest.equals(remaining)) {
					// significa que, aun cogiendo los elementos del subconjunto
					// el remaininng se queda igual
					alternativas = List2.of(0);
				} else {
	...
	public SubconjuntosProblem neighbor(Integer a) {
		// si a = 0, no cojo el subconjunto, dejo el remaining igual (le hago la copia)
		// si a = 1, cojo el subconjunto y me quedo con la diferencia, es decir, 
		// lo que me queda por cubrir
